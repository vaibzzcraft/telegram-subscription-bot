...
def cb_plan(call):
    plan_key = call.data[5:]          # "1week" or "1month"
    plan     = PLANS[plan_key]
    amount   = plan["price"]
...
def cb_approve(call):
    ...
    # Create invite link (single-use, member limit 1)
    try:
...
def check_expired_subscriptions():
    """Runs every 30 minutes. Removes expired users from channel and notifies them."""
    while True:
        time.sleep(30 * 60)  # 30 minutes
        now = datetime.utcnow()

        expired_users = users.find({"expiry": {"$lt": now}, "active": True})

        for rec in expired_users:
            user_id = rec["user_id"]
            ...
            try:
                ...
                time.sleep(1)
                bot.unban_chat_member(CHANNEL_ID, user_id)  # unban so they can rejoin later
            except Exception as e:
...
if __name__ == "__main__":
    ...
    t = threading.Thread(target=check_expired_subscriptions, daemon=True)
    t.start()
